...
def compose_svg(atoms_to_put, options):
    ...
    for put_atom in atoms_to_put:
        if isinstance(put_atom, PutAnnotation):
            ...
            annotation_scale = options.annotation_scale if put_annotation.allow_scaling else 1.0
            if put_atom.above_pieces:
    ...
    if options.debug:
        for x_rel in (0.0, 1.0):
            for y_rel in (0.0, 1.0):
                jobs_above_piece_level.append((x_rel, y_rel, os.path.join(options.piece_theme_dir, _DIAMOND_FILE_NAME), 1.0))
