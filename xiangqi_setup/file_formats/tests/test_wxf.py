...
class PlayerRelativeViewTest(TestCase):
    ...
    def test_further_up_the_board_sort_key(self, party, expected_furthest_up_the_board_x_y):
        ...
        x_y_pairs = [(2, 2), (2, 4), (2, 1),
                     (2, 3)]  # min and max are not at the so they move during sorting
        x_y_put_pieces = [PutPiece(party, piece, x, y) for x, y in x_y_pairs]
        expected_piece_furthest_up_the_board = PutPiece(party, piece,
...
class BoardTest(TestCase):
    ...
    def test_calculate_destination_of_move(self, piece, party, cur_x_y, operator, argument,
                                           expected_new_x_y):
